...
pdf(here("csv_and_pdf_output", "Industry_forecasts.pdf"), onefile = TRUE, height=8.5, width=11)
nested%>%
  select(plots)%>%
  walk(print)
dev.off()
# write the wide format data to disk--------
write_csv(wide, here("csv_and_pdf_output", paste0("LMO_", forecast_start, "_industry_forecast.csv")), na ="")
