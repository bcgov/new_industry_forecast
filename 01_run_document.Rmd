...
driver_data <- read_excel(here("data","current","Driver-based forecast 2023 Feb 22.xlsx"))%>%
...
default_industry <- industries[!industries %in% unique(forecast_already$industry)][1] #for industry to display
...
if (nrow(forecast_already > 0)) {
  ...
  forecasted_by_year <- forecast_already %>%
    ...
    select(-total, -employment, -growth)
} else {
