...
- data/current/last_year_from_stokes.xlsx
...
old <- readxl::read_excel(here("data","current","last_year_from_stokes.xlsx"), skip = 3)|>
...
driver_data <- read_excel(here("data","current","driver_2025_partial.xlsx"))%>%  
  ...
  unite(industry, code, industry, sep=": ")%>%
  mutate(year=as.numeric(year),
         value=value*1000 #driver data was in 1000's this year
         ) 
...
reactive_new <- reactive({
  adjustment_vector <- c(input$first, input$second, input$third, input$fourth, input$fifth, input$sixth, 
                         input$seventh, input$eighth, input$ninth, input$tenth, input$eleventh)
  ...
  base_data <- switch(input$fcast,
    "Last Year" = reactive_old() %>% mutate(year = year + 1),
