...
historic_start <- 2010
historic_end <- 2019
colors <- c("Employment" = "black", "Last Year" = "purple", "New Forecast" = "orange", "Driver Data" = "blue", "Forecast Already" = "red", "Proportion Forecast"="aquamarine")
...
prop_fcast <- read_csv(here("data", "current","prop_forecast.csv"))|>
  filter(year>=min(driver_data$year))|>
  separate(industry, c("wrong_code", "industry"), sep=": ")%>%
  stringdist_right_join(employment)%>% #don't trust industry names: manually check the join
  ungroup()|>
  select(-employment, -industry.x)%>%
  rename(industry=industry.y)%>%
  unite(industry, code, industry, sep=": ")%>%
...
reactive_new <- reactive({
  ...
  input$fcast=="Driver" ~ reactive_driver()%>%
    ...
    select(-slope, -adjustment_vector),
  input$fcast=="Proportion"~ reactive_prop()%>%
    mutate(value = value * input$shift) %>%
    full_join(adjustments) %>%
    mutate(
      value = value * slope,
...
renderPlot({
  ggplot() +
    ...
    geom_line(data = reactive_driver(), mapping = aes(year, value, colour = "Driver Data"), lwd = 3) +
    geom_line(data = reactive_prop(), mapping = aes(year, value, colour = "Proportion Forecast"), lwd = 3) +
