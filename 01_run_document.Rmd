...
sim_fcast <- read_csv(here("data","current","sim_fcasts.csv"))|>
  rename(value=mean_fcast) #sim forecast based on employment file above... has correct names (in proportions)

old_forecast_wrong_names <- readxl::read_excel(here("data","current","last_year_from_stokes.xlsx"), skip = 3)|>
  janitor::remove_constant()|>
  pivot_longer(cols=-Industry, names_to = "year")|>
  mutate(year=as.numeric(year))|>
  clean_names()

forecast_years <- old_forecast_wrong_names|> # the years to be forecast
  distinct(year) |>
  pull(year) + 1
...
driver_data <- read_excel(here("data","current","driver_2025_partial.xlsx"))|>  
  ...
  stringdist_right_join(employment)|> #CHECK THE JOIN
  ungroup()|>
...
budget_constraint <- read_xlsx(here("data","current", "constraint.xlsx"))
...
#join budget_plus_extrapolation and sim proportions to form sim forecast

sim_fcast <- inner_join(sim_fcast, budget_plus_extrapolation)|>
